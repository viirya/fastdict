...
d_ptr = drv.mem_alloc(1 * 10)
drv.memcpy_htod(int(d_ptr), np.zeros(5, dtype=np.uint8))
drv.memcpy_htod(int(d_ptr) + 5, np.zeros(5, dtype=np.uint8))
...
twod_gpu = drv.mem_alloc(4 * 8)
address = np.array([int(a1_addr), int(b1_addr), int(c1_addr), int(d_ptr)]).astype(np.uint64)
print address
print int(twod_gpu)
