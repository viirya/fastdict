...
__global__ void compressed_hamming_dist(uint64_t* query, uint64_t** bit_counts, uint64_t* max_length, uint64_t* distances)
{
    ...
    __shared__ uint32_t bit_counts_scan_bases[64];
    __shared__ uint8_t bit_types_for_scan_bases[64];
    __shared__ uint64_t accu_bit_counts_for_scan_bases[64];
    ...
    if (thread_column_index < 64) {
        ...
        uint32_t smallest_binary_code_index = (gridDim.x * blockDim.x * blockIdx.y + blockIdx.x * blockDim.x) * batch_size;
        ...
        while (count_for_bits <= max_length[0]) {
            ...
            if (count_for_bits > smallest_binary_code_index) {
                bit_counts_scan_bases[thread_column_index] = bit_count_index;
                bit_types_for_scan_bases[thread_column_index] = bit_type;
                accu_bit_counts_for_scan_bases[thread_column_index] = count_for_bits - bit_counts[thread_column_index][bit_count_index];
                break;
            }
    ...
    if (i_for_batch < max_length[0]) {
        ...
        for (uint64_t column_index = 0; column_index < 64; column_index++) {
            ...
            uint64_t count_for_bits = accu_bit_counts_for_scan_bases[column_index];
            uint64_t bit_count_index = bit_counts_scan_bases[column_index];
            uint8_t bit_type = bit_types_for_scan_bases[column_index];
